...
// 1. Make getScopeFilter async and implement full hierarchy trace
...
// 2. Await getScopeFilter in GET /api/init
...
content = content.replace(
  ...
  try {
    ...
    if (!scope.memory(installation) && !scope.memory(updatedPipe)) {
      return res.status(403).json({ error: 'Cannot create records outside your scope' });
    }`
);
...
// Update memory fallback for PUT /api/installations/:pipeId
content = content.replace(
  ...
  `const scope = await getScopeFilter(req.user);
      ...
      if (targetIndex !== -1 && scope.memory(inMemoryData.installations[targetIndex])) {
        ...
        return res.status(403).json({ error: 'Permission denied or record not found' });
      }`
);
...
content = content.replace(
  ...
  try {
    const scope = await getScopeFilter(req.user);
    // Since monitoring is created, we can check if the pipe ID or record falls in scope
    if (!scope.memory(record)) {
      return res.status(403).json({ error: 'Cannot monitor records outside your scope' });
    }`
);
